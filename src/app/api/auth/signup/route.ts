...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      existingUser = await convexClient.query('users:getUserByEmail', {
    ...
    let userId: string
    try {
      userId = await convexClient.mutation('users:createUserWithPassword', {
        name: name.trim(),
        email: normalizedEmail,
        passwordHash,
      })
    ...
    try {
      await convexClient.mutation('sessions:createSession', {
        userId,
        token: sessionToken,
    ...
    let userData: any
    try {
      userData = await convexClient.query('users:getUser', { userId })
    } catch (getUserError) {
