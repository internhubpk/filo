...
export default defineSchema({
  // Users table
  // NOTE (payments removed): the former `paymentVerifications`, `payments`,
  // `subscriptions` and `webhookEvents` tables were dropped together with the
  // SafePay/manual-payment flow. Quota enforcement now lives purely in
  // `usageRecords` + `users.planId`.
  ...
  users: defineTable({
    ...
    // Legacy field from the removed Safepay integration. Kept optional so
    // historical documents remain schema-valid; nothing writes to it anymore.
    providerCustomerId: v.optional(v.string()),
    // Payments have been removed entirely: every new signup is activated
    // instantly (status = "active") and can generate right away.
    // "suspended" revokes access (admin action).
    status: v.union(
